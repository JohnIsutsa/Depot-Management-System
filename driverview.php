<?php 
	...
	$start = ($page - 1) * $limit;
	$result = $link->query("SELECT * FROM drivers LIMIT $start, $limit");
	$drivers = $result->fetch_all(MYSQLI_ASSOC);

	$result1 = $link->query("SELECT count(driver_id) AS id FROM drivers");
    ...
    if(isset($_POST['search'])){
        ...
        $query = "SELECT * FROM `drivers` 
            WHERE CONCAT(`driver_id`,`driver_name`,`national_id`) LIKE '%".$valueToSearch."%'";
...
<body>
    ...
    <br>
			...
			<div class="col-md-10">
				<nav aria-label="Page navigation">
				    ...
				    <li>
				      <a <?php if ($Previous == 0) { ?>
                          href="driverview.php"
                     <?php } else { ?> href="driverview.php?page=<?= $Previous; ?>" <?php } ?> aria-label="Previous">
				        <span aria-hidden="true">&laquo; Previous</span>
				      </a>
				    </li>
				    <?php for($i = 1; $i<= $pages; $i++) : ?>
				    	<li><a href="driverview.php?page=<?= $i; ?>"><?= $i; ?></a></li>
				    <?php endfor; ?>
				    <li>
				      <a href="driverview.php?page=<?= $Next; ?>" aria-label="Next">
        ...
        <button id="exportButton" onclick="exportTableToExcel('table', 'driver-data')" style="color: white; background-color: green;" class="btn btn-lg clearfix"><span class="fa fa-file-excel-o"></span> Export to Excel</button>

        <br> <br> 
		<div style="height: 600px; overflow-y: auto;">
            <form action="driverview.php" method="post">
