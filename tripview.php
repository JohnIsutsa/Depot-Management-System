<?php 
	...
	$start = ($page - 1) * $limit;
	$result = $link->query("SELECT * FROM driver_trips LIMIT $start, $limit");
	$drivers = $result->fetch_all(MYSQLI_ASSOC);

	$result1 = $link->query("SELECT count(trip_id) AS id FROM driver_trips");
    ...
    if(isset($_POST['search'])){
        ...
        $query = "SELECT * FROM `driver_trips` 
            WHERE CONCAT(`trip_id`,`driver_name`,`national_id`,`trip_date`,`trip_from`,`trip_destination`,`trip_operation`,`container_number`,`container_size`, `shipping_line`, `trip_date`) LIKE '%".$valueToSearch."%'";
...
<body>
    ...
    <br>
			...
			<div class="col-md-10">
				<nav aria-label="Page navigation">
				    ...
				    <li>
				      <a <?php if ($Previous == 0) { ?>
                          href="tripview.php"
                     <?php } else { ?> href="tripview.php?page=<?= $Previous; ?>" <?php } ?> aria-label="Previous">
				        <span aria-hidden="true">&laquo; Previous</span>
				      </a>
				    </li>
				    <?php for($i = 1; $i<= $pages; $i++) : ?>
				    	<li><a href="tripview.php?page=<?= $i; ?>"><?= $i; ?></a></li>
				    <?php endfor; ?>
				    <li>
				      <a href="tripview.php?page=<?= $Next; ?>" aria-label="Next">
        ...
        <button id="exportButton" onclick="exportTableToExcel('table', 'trip-data')" style="color: white; background-color: green;" class="btn btn-lg clearfix"><span class="fa fa-file-excel-o"></span> Export to Excel</button>

        <br> <br> 
		<div style="height: 600px; overflow-y: auto;">
            <form action="tripview.php" method="post">
                ...
                <input type="submit" name="search" value="Filter"><br><br>
                    ...
                    <td><?php echo $row['trip_from'];?></td>

                    <td><?php echo $row['trip_destination'];?></td>

                    <td><?php echo $row['trip_operation'];?></td>
